/*
 * @Author: Weiqi Si 
 * @Date: 2018-09-27 16:10:51 
 * @Last Modified by: Weiqi Si
 * @Last Modified time: 2018-09-27 16:27:25
 */
...
int content_parse(char* content_output, const char *content_input, const char *device_add)
{
	...
	/* lang输入 */
	if(find_device_cnt(device_add,"lang")==0)
	{
		//return 0;
	}
	else
	...
	/* cuid输入 */
	if (find_device_cnt(device_add, "cuid") == 0)
	{
		//return 0;
	}
	else
	...
	/* screenPath输入 */
	if (find_device_cnt(device_add, "screenPath") == 0)
	{
		//return 0;
	}
	else
...
int main(void)
{
	...
	if (strcmp(method, "POST") == 0)
	{
		...
		if (len != NULL)
		{
			...
			{
				printf("<p style='font-weight:bold'>EDIT_DEVICE_ERROR</p>");
				return 0;
			}
			else
			{
				printf("<p style='font-weight:bold'>EDIT_DEVICE_SUCCEED</p>");
