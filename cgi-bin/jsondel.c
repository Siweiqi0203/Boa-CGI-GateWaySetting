/*
 * @Author: Weiqi Si 
 * @Date: 2018-09-27 16:28:08 
 * @Last Modified by: Weiqi Si
 * @Last Modified time: 2018-09-27 17:03:20
 */
...
int content_parse(char* content_output, const char *content_input, const char *device_del)
{
	char *pstart, *pend;
	...
	int  device_cnt = 0;
	int  del_num_int,del_num_len;

	/* 过滤无用字节 */
	pstart = (char*)device_del;
	char del_num_char[MAX_DEVICE_KEY_LEN] = { 0 };
	...
	del_num_len = pend-(pstart+1);
	// 存入del_num_char数组
	strncpy(del_num_char, pstart+1, del_num_len);
	del_num_int = atoi(del_num_char);
...
int main(void)
{
	...
	if (strcmp(method, "POST") == 0)
	{
		...
		if (len != NULL)
		{
			...
			{
				printf("<p style='font-weight:bold'>DELETE_DEVICE_ERROR</p>");
				return 0;
			}
			else
			{
				printf("<p style='font-weight:bold'>DELETE_DEVICE_SUCCEED</p>");
